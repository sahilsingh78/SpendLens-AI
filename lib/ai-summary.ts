...
- Total current AI spend: ${formatCurrencyFull(
    audit.totalMonthlySpend
  )}/month
- Total potential savings: ${formatCurrencyFull(
    audit.totalMonthlySavings
  )}/month (${audit.savingsPercentage}%)
- Annual savings opportunity: ${formatCurrencyFull(
...
Do NOT:
- Use bullet points
- Mention Credex unless credits are recommended
`;
}
...
function buildFallbackSummary(
  ...
  if (
    ...
    return `Your ${input.teamSize}-person team is spending ${formatCurrencyFull(
      totalMonthlySpend
    )}/month on AI tools efficiently. Across ${
  ...
  return `Your ${input.teamSize}-person team is currently spending ${formatCurrencyFull(
    ...
    totalMonthlySavings
  )}/month available to recapture. The largest optimization opportunity comes from ${
    topRec.toolName
  }, where ${
    topRec.reason
  } Implementing the recommended changes could recover ${formatCurrencyFull(
    totalAnnualSavings
  )} annually and improve overall tool efficiency without disrupting workflows.`;
}
